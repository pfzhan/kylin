...
function main() {
  ...
  case $1 in
    "init" )
        ...
        ${PROJECT_DIR}/build/script_newten/download-spark.sh
        if [[ $? != 0 ]]; then
        ...
        eval "cat <<EOF
            $(<"${WORKDIR}/runConfigurations/BootstrapServer_sandbox_.xml")
EOF" >"${PROJECT_DIR}/.idea/runConfigurations/BootstrapServer_sandbox_.xml"
...
EOF"
    ...
    "sample" )
        ...
        docker compose -f "${WORKDIR}/docker-compose.yml" exec datanode bash -c "hadoop dfs -mkdir -p /tmp/sample_cube/data \
            && hadoop dfs -put /tmp/ssb/* /tmp/sample_cube/data/"
