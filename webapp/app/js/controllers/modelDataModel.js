...
KylinApp.controller('ModelDataModelCtrl', function ($location,$scope, $modal,cubeConfig,MetaModel,SweetAlert,ModelGraphService,$log,TableModel,ModelService,loadingRequest,modelsManager,VdmUtil) {
    ...
    $scope.initUsedLookup = function(){
      ...
      angular.forEach($scope.usedMeasuresCubeMap,function(cube,measureName){
      var tableName=VdmUtil.getNameSpace(measureName);
      $scope.selectedTableCubeMap[tableName]=$scope.selectedTableCubeMap[tableName]||[];
        angular.forEach(cube,function(cubeName){
          $scope.selectedTableCubeMap[tableName].push(cubeName);
        });
      });
    ...
    $scope.changeFactTable = function () {
        $scope.FactTable.root=VdmUtil.removeNameSpace($scope.modelsManager.selectedModel.fact_table);
        $scope.aliasTableMap[$scope.FactTable.root]=$scope.modelsManager.selectedModel.fact_table;
        $scope.aliasName.splice(0,$scope.aliasName.length);
        $scope.aliasName.push($scope.FactTable.root);
