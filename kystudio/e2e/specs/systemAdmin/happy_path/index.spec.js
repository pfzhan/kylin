...
const { login, logout, addUser, searchCurProject, setUserToProject, changeToCurProject, addTableLink, addMeasure } = require('../../utils/businessHelper')
const { changeFormInput, changeFormTextarea, hoverOn, changeFormSelect, dragAndDrop } = require('../../utils/domHelper')
...
describe('系统管理员创建项目', async function () {
  ...
  // 创建项目
  it('创建项目', async () => {
    // 如果出现更新许可证弹框，点击取消，关闭弹框
    // const licenseClass = '.kapApp > div > div > div > .updateKAPLicense'
    // await driver.wait(until.elementLocated(By.css(licenseClass)), 10000)
    // await driver.findElement(By.css(`${licenseClass} .el-dialog__close`)).click()
    // await driver.wait(until.elementIsNotVisible(await driver.findElement(By.css(licenseClass))), 10000)

    // 统一调用登录
    await login(driver, USERNAME_ADMIN, PASSWORD_ADMIN);
    // await driver.findElement(By.css('.errMsgBox .dialog-footer .el-button--default')).click() // 解决4.1 分支接口404报错弹窗出现问题
  ...
  //   await driver.wait(until.elementIsNotVisible(driver.findElement(By.css('.user-edit-modal'))), 10000)
  ...
  //   // 点击+用户/用户组
  //   const addUserBtnClass = '#projectAuth .ksd-fleft .el-button--primary:nth-child(2)'
  //   await driver.wait(until.elementLocated(By.css(addUserBtnClass)), 10000)
  //   await driver.findElement(By.css(addUserBtnClass)).click()
  //   await driver.sleep(2000)
  //   // await driver.wait(until.elementIsVisible(driver.findElement(By.css('author_dialog'))), 10000)
  //   const addIconClass = '.author_dialog .user-group-select .el-icon-ksd-add_2'
  ...
  //   // 项目 admin
  //   await setUserToProject(driver, USERNAME_PROJECT_ADMIN, 3, 2)
  //   // 项目 management
  //   await setUserToProject(driver, USERNAME_PROJECT_MANAGEMENT, 4, 4)
  //   // 项目 operation
  //   await setUserToProject(driver, USERNAME_PROJECT_OPERATION, 5, 3)
  //   // 项目 query
  //   await setUserToProject(driver, USERNAME_PROJECT_QUERY, 6, 1)
