    ...
    <el-row :gutter="10" id="listBox">
      <el-col :span="showStep?19:24" id="leftTableBox" :class="{'show-steps': showStep}">
        <el-table class="ksd-el-table jobs-table"
          ...
          <el-table-column
            width="180"
            :filters="allStatus.map(item => ({text: $t(item.text), value: item.value}))" :filtered-value="filter.status" :label="$t('ProgressStatus')" filter-icon="el-ksd-icon-filter_22" :show-multiple-footer="false" :filter-change="(v) => filterContent(v, 'status')">
            <template slot-scope="scope">
  ...
  filterContent (val, type) {
    ...
    val.length && val.forEach(item => {
      if (!list.includes(item)) {
        this.filterTags.push({label: type === 'status' && item === 'STOPPED' ? 'PAUSED' : item, source: maps[type], key: type})
      }
    })
