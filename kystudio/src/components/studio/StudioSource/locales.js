export default {
  'en': {
    ...
    dropTabelDepen: 'This source table {tableName} is referenced by models, whose total storage size is {storageSize}. Please confirm whether to only delete this table or delete all.<br/><br/>Delete Table: Only deleting this source table and stopping related jobs. This will cause models (indexes) being broken. You can reload this table to fix them.<br/><br/>Delete All: This source table{dropTabelDepenSub2}, and the referenced models would be deleted{dropTabelDepenSub}. The related jobs would be stopped.',
    dropTabelDepen2: 'This source table {tableName} is referenced by models or snapshot, whose total storage size is {storageSize}. Please confirm whether to only delete this table or delete all.<br/><br/>Delete Table Only: This source table and the snapshot would be deleted, while the referenced models would be kept (BROKEN). The related jobs would be stopped.<br/><br/>Delete All: This source table{dropTabelDepenSub2}, the snapshot, and the referenced models would be deleted{dropTabelDepenSub}. The related jobs would be stopped.',
    dropTabelDepenSub: ', while the attached Hive table will be kept',
  ...
  'zh-cn': {
    ...
    dropTabelDepen: '该源表 {tableName} 正在被模型引用，总存储大小为 {storageSize}。请确认仅删除源表或全部删除。<br/><br/>删除源表：仅删除源表信息，并停止相关任务。删除后将导致对应模型变为不可用状态，您可以通过再次添加该表进行修复。<br/><br/>全部删除：删除源表{dropTabelDepenSub2}、相关模型（索引组）{dropTabelDepenSub}，并停止相关任务。',
    dropTabelDepen2: '该源表 {tableName} 正在被模型或快照引用，总存储大小为 {storageSize}。请确认仅删除源表或全部删除。<br/><br/>仅删除源表：删除源表、快照，但保留模型（变为 BROKEN 状态），表的相关任务将停止。<br/><br/>全部删除：删除源表{dropTabelDepenSub2}、快照、相关模型{dropTabelDepenSub}，并停止相关任务。。',
    dropTabelDepenSub: '，但保留关联的 Hive 表',
    dropTabelDepenSub2: '、关联的 Kafka 表',
    delKafkaTableAllTips: '将删除源表和相关模型（索引组），但保留关联的 Hive 表，并停止相关任务。确定要全部删除吗？',
    delHiveTableAllTips: '将删除源表、关联的 Kafka 表和相关模型（索引组），并停止相关任务。确定要全部删除吗？',
    dropTabelDepen3: '该源表 {tableName} 正在被快照引用，总存储大小为 {storageSize}。请确认是否删除表及其快照。',
