export default {
  ...
  'zh-cn': {
    ...
    dropTabelDepen: '该源表 {tableName} 正在被模型引用，总存储大小为 {storageSize}。请确认仅删除源表或全部删除。<br/><br/>删除源表：删除源表，并停止相关任务。但保留{dropTabelDepenSub}相应模型（将变为 Broken 状态，可通过重新添加该表进行修复）。<br/><br/>全部删除：删除源表{dropTabelDepenSub2}、相关模型，并停止/删除相关任务。{dropTabelDepenSub3}',
    dropTabelDepen2: '该源表 {tableName} 正在被模型或快照引用，总存储大小为 {storageSize}。请确认仅删除源表或全部删除。<br/><br/>删除源表：删除源表、快照，并停止相关任务。但保留{dropTabelDepenSub}相应模型（将变为 Broken 状态，可通过重新添加该表进行修复）。<br/><br/>全部删除：删除源表、快照{dropTabelDepenSub2}、相关模型，并停止/删除相关任务。{dropTabelDepenSub3}',
    dropTabelDepenSub: '关联的 Hive 表和',
    dropTabelDepenSub2: '、关联的 Kafka 表',
    dropTabelDepenSub3: '但保留关联的 Hive 表。',
