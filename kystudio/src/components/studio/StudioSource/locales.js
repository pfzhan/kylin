export default {
  'en': {
    ...
    dropTabelDepen: 'This source table {tableName} is referenced by models, whose total storage size is {storageSize}. Please confirm whether to only delete this table or delete all.<br/><br/>Delete Table: Only delete this source table, and stop related jobs. While {dropTabelDepenSub} the referenced models will be kept (BROKEN, can be fixed by reloading the table).<br/><br/>Delete All: Delete this source table, attached Kafka table, the referenced models, and {dropTabelDepenSub3}delete related jobs. {dropTabelDepenSub4}',
    ...
    dropTabelDepenSub3: 'stop/',
    dropTabelDepenSub4: 'While the attached Hive table will be kept.',
  ...
  'zh-cn': {
    ...
    dropTabelDepen: '该源表 {tableName} 正在被模型引用，总存储大小为 {storageSize}。请确认仅删除源表或全部删除。<br/><br/>删除源表：删除源表，并停止相关任务。但保留{dropTabelDepenSub}相应模型（将变为 Broken 状态，可通过重新添加该表进行修复）。<br/><br/>全部删除：删除源表{dropTabelDepenSub2}、相关模型，并{dropTabelDepenSub3}删除相关任务。{dropTabelDepenSub4}',
    dropTabelDepen2: '该源表 {tableName} 正在被模型或快照引用，总存储大小为 {storageSize}。请确认仅删除源表或全部删除。<br/><br/>删除源表：删除源表、快照，并停止相关任务。但保留{dropTabelDepenSub}相应模型（将变为 Broken 状态，可通过重新添加该表进行修复）。<br/><br/>全部删除：删除源表、快照{dropTabelDepenSub2}、相关模型，并{dropTabelDepenSub3}删除相关任务。{dropTabelDepenSub4}',
    dropTabelDepenSub: '关联的 Hive 表和',
    dropTabelDepenSub2: '、关联的 Kafka 表',
    dropTabelDepenSub3: '停止/',
    dropTabelDepenSub4: '但保留关联的 Hive 表。',
