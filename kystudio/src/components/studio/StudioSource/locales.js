export default {
  ...
  'zh-cn': {
    ...
    dropTabelDepen2: '该源表 {tableName} 正在被模型和快照引用，总存储为 {storageSize}。请确认仅删除该表或全部删除<br/><br/>删除源表：仅删除源表信息，并停止相关任务。删除后将导致对应模型变为不可用状态，您可以通过再次添加该表进行修复。<br/><br/>全部删除：删除源表及其相关模型（索引组），并停止相关任务。',
