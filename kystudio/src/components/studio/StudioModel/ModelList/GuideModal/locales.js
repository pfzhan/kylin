export default {
  'en': {
    dimMeasTitle: 'Dimensions and Measures Settings',
    dimMeasDesc: 'To be used for indexes, please set dimensions and measures here after adding the tables.',
    iKnow: 'I Know',
    segmentTitle: 'Define the data range for serving queries 1/3',
    segmentSubTitle: 'Add segments to define the model’s data range for serving queries.',
    ignore: 'Ignore',
    next: 'Next',
    pre: 'Previous',
    indexOverview: 'Index Overview',
    aggregateGroup: 'Aggregate Group',
    tableIndex: 'Table Index',
    indexTitle: 'Add indexes to improve query performance 2/3',
    indexSubTitle: 'Set dimensions and measures in aggregate groups and table indexes according to query requirements.',
    buildTitle: 'Indexes could be used for queries after being built 3/3',
    buildSubTitle: 'Build indexes according to the desired segments. For those segments without indexes, queries would be answered by push down.'
  },
  'zh-cn': {
    dimMeasTitle: '维度、度量设置',
    dimMeasDesc: '添加表之后，建议您添加维度和度量，以便后续设置索引。',
    iKnow: '知道了',
    segmentTitle: '定义可服务查询的数据范围 1/3',
    segmentSubTitle: '添加 Segment 来为模型定义可服务查询的数据范围',
    ignore: '忽略',
    next: '下一步',
    pre: '上一步',
    indexOverview: '索引总览',
    aggregateGroup: '聚合组',
    tableIndex: '明细索引',
    indexTitle: '添加索引提高查询效率 2/3',
    indexSubTitle: '按照查询特征，将维度和度量设置在聚合组和明细索引中。',
    buildTitle: '构建后索引可用于查询 3/3',
    buildSubTitle: '你需要将索引按照数据范围（Segment）进行构建，构建完成后索引可回答查询。对于没有索引的 Segments, 查询将通过下压回答。'
  }
}
