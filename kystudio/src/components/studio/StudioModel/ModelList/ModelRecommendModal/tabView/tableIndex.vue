...
<script>
  ...
  @Component({
    ...
    methods: {
      ...mapActions({
        getTableIndex: 'GET_TABLE_INDEX_CONTENTLIST'
      }),
      ...mapMutations({
      }),
      rowKey (row) {
  ...
  export default class recomTableIndex extends Vue {
    ...
    ST = null
    expandRows = []
    perPageSize = +localStorage.getItem(this.pageRefTags.tableIndexPager) || pageCount

    /* @Watch('list', { deep: true })
    onListChange (val) {
      // this.tableShowList = objectClone(val)
      this.tableShowList = val
      renderTableColumnSelected(this.tableShowList, this, 'tableIndexList')
    } */
    mounted () {
    ...
    async loadTableIndexList (row, page) {
      try {
        let params = {
        ...
        for (let i = 0; i < result.columns.length; i++) {
          let obj = {
            ...
            shared: result.shard_by_columns.indexOf(result.columns[i]) > -1
          }
    ...
    expandChange (row, expandedRows) {
      ...
      if (inExpand.length > 0) { // 说明是展开，需要发接口
        this.loadTableIndexList(row, {curpage: 0})
      }
    }
    ...
    selectionAllChange (selection) {
      ...
      this.$emit('tableIndexSelectedChange', {type: 'table_index', selectType: selectAllType})
    }

    // 单元行的选中/取消
    selectionChange (selection, row) {
      row.isSelected = !row.isSelected
      this.$emit('tableIndexSelectedChange', {type: 'table_index', selectType: 'single', data: row})
    }
    ...
    handleCurrentChange (currentPage, row) {
      this.loadTableIndexList(row, {curpage: currentPage})
    }
