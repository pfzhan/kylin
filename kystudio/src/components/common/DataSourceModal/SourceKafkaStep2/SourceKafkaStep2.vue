...
export default class SourceKafkaStep2 extends Vue {
  ...
  convertJson (columnData) {
    ...
    // 暂时不需要默认新增的streamingAutoGenerateMeasure列
    // for (var i = 0; i < this.streamingAutoGenerateMeasure.length; i++) {
    //   var defaultCheck = 'Y'
    //   const index = indexOfObjWithSomeKeyByfuzzy(columnList, 'name', this.streamingAutoGenerateMeasure[i].name)
    //   if (index !== -1) {
    //     columnList[index].isReName = true
    //     this.reNameList.push(columnList[index].name)
    //   }
    //   columnList.push({
    //     'attribute': this.streamingAutoGenerateMeasure[i].name,
  ...
  loadColumnZH () {
    ...
    //   if (column.checked === 'Y' && column.fromSource === 'Y' && column.type === 'timestamp' && !!column.name.trim()) {
    ...
    //   this.$refs.kafkaForm.validateField('timestampField')
    // } else {
