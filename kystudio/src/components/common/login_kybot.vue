<template>
  <div class="login_kybot">
    <p style="margin-bottom:20px;"><span class="el-dialog__title">{{$t('kyAccount')}}</span></p>
    <el-form :model="kyBotAccount" :rules="rules" ref="loginKybotForm" >
      <el-form-item prop="username">
        <el-input v-model="kyBotAccount.username" :placeholder="$t('kylinLang.common.username')" auto-complete="new-password"></el-input>
      </el-form-item>
      <el-form-item prop="password">
        <el-input v-model="kyBotAccount.password" type="password" :placeholder="$t('kylinLang.common.password')" auto-complete="new-password"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button @click="loginKyBot" :loading="loginLoading" type="primary" class="btn-loginKybot">{{$t('login')}}</el-button>
      </el-form-item>
    </el-form>
    <p class="no-account" style="color: #0988DE;"><span>{{$t('noAccount')}}?</span> <a @click="signUp" target="_blank">{{$t('singUp')}}</a></p>
  </div>
</template>
<script>
  ...
  export default {
    ...
    locales: {
      'en': {singUp: 'Sign Up Now', noAccount: "Don't have an account", usernameEmpty: 'Please enter username', noUserPwd: 'Password required', login: 'Login', openSuccess: 'Login Success', kyAccount: 'Kyligence Account'},
      'zh-cn': {singUp: '立即注册', noAccount: '还没有账号', usernameEmpty: '请输入用户名', noUserPwd: '密码不能为空', login: '登录', openSuccess: '登录成功', kyAccount: 'Kyligence 帐号'}
    }
  }
</script>
<style lang="less">
