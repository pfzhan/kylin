  ...
  <el-dialog class="group-edit-modal" :width="modalWidth"
    ...
    <el-form :model="form" @submit.native.prevent :rules="rules" ref="form" v-if="isFormShow">
      ...
      <el-form-item :label="$t('kylinLang.common.groupName')" prop="group_name" v-if="isFieldShow('group_name')">
        <el-input auto-complete="off" @input="value => inputHandler('group_name', value)" :value="form.group_name" @keyup.enter.native="submit"></el-input>
      </el-form-item>
      <!-- 表单：分配用户 -->
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" size="medium" :loading="submitLoading" :disabled="submitLoading" @click="submit">{{$t('kylinLang.common.save')}}</el-button>
    </div>
  </el-dialog>
</template>
