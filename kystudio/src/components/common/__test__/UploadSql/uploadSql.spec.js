...
describe('Component SuggestModel', () => {
  ...
  it('methods events', async () => {
    ...
    expect($message.error).toBeCalledWith('Invalid file format。')
    expect(wrapper.vm.fileSizeError).toBeFalsy()

    wrapper.vm.handleRemove({}, [])
    expect(wrapper.vm.uploadItems).toEqual([])
    wrapper.vm.handleRemove({}, [{size: 6 * 1024 * 1024, file: null}])
    expect($message.warning).toBeCalledWith('The total file size cannot exceed 5 MB')
    expect(wrapper.vm.fileSizeError).toBeTruthy()
    ...
    expect(handleSuccess).toBeCalled()
    expect(wrapper.vm.$data.uploadFlag).toBe('step2')
    expect(wrapper.vm.whiteSqlData).toEqual({'blacklist': 1, 'capable': true, 'capable_sql_num': 1, 'data': [{'capable': true, 'id': 0, 'sql': 'sql1', 'sql_advices': []}], 'imported': 2, 'msg': 'over sizes', 'size': 100})
    expect(wrapper.vm.selectSqls).toEqual([{'capable': true, 'id': 0, 'sql': 'sql1', 'sql_advices': []}])
    ...
    jest.runAllTimers()
    expect(wrapper.vm.filteredDataSize).toBe(1)
