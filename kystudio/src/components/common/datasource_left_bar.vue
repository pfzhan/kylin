...
<script>
  ...
  export default {
    ...
    locales: {
      ...
      'zh-cn': {dataSourceTypeCheckTip: '请选择数据源类型', singleSourceTip: '每个项目支持一种数据源类型，选定后将无法修改。', newDataSource: '新数据源', loadhiveTables: '加载Hive表元数据', loadTables: '加载表元数据', loadKafkaTopic: '设置Kafka主题', selectedHiveValidateFailText: '请输入完整表名\'database.table\'。', dialogHiveTreeLoading: '加载中', loadHiveTip: '<p style="font-weight: bold">加载表元数据的方式</p><p class="ksd-mt-10">选择表：可以在左侧的元数据树选择需要加载的表。每次最多可加载1000张表。</p><p>输入表：可以在右侧的加载框中直接输入‘database.table’，按回车键结束输入。输入多张表名时，请使用逗号分隔，每次最多可加载1000张表。</p>', sampling: '表采样'}
    }
  }
</script>
