  ...
  <div class="ksd_left_bar" id="input-inner">
    ...
    <el-dialog class="load-datasource-dialog" :title="dataSourceLoadDialogTitle" width="720px" limited-area :visible.sync="loadDataSourceVisible" :close-on-press-escape="false" :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click="nextLoadForm"  v-if="activeLoadFormIndex===-1">{{$t('kylinLang.common.next')}}</el-button>
        <el-button @click="loadHiveList" :loading="loadHiveLoad" v-if="(currentUserCheckType==='0' || currentUserCheckType==='16'  || currentUserCheckType==='8')&& activeLoadFormIndex!==-1">{{$t('kylinLang.common.sync')}}</el-button>
        <el-button @click="checkKafkaForm" :loading="kafkaLoading" v-if="currentUserCheckType==='1' && activeLoadFormIndex!==-1">{{$t('kylinLang.common.submit')}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    locales: {
      ...
      'zh-cn': {dataSourceTypeCheckTip: '请选择数据源类型', singleSourceTip: '每个项目支持一种数据源类型，选定后将无法修改。', newDataSource: '新数据源', loadhiveTables: '加载 Hive 表元数据', loadTables: '加载表元数据', loadKafkaTopic: '设置 Kafka 主题', selectedHiveValidateFailText: '请输入完整表名\'database.table\'。', dialogHiveTreeLoading: '加载中', loadHiveTip: '<p style="font-weight: bold">加载表元数据的方式</p><p class="ksd-mt-10">选择表：可以在左侧的元数据树选择需要加载的表。每次最多可加载 1000 张表。</p><p>输入表：可以在右侧的加载框中直接输入 ‘database.table’，按回车键结束输入。输入多张表名时，请使用逗号分隔，每次最多可加载 1000 张表。</p>', sampling: '表采样'}
    }
  }
</script>
