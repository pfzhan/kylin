  ...
  <aside class="data-source-bar" :style="dataSourceStyle">
    ...
    <section class="body">
      ...
      <div v-if="showTreeFilter" class="ksd-mb-16 dispaly-flex">
        ...
        <div class="add-source-table-icon" v-if="isShowLoadTableInnerBtn&&$store.state.config.platform !== 'iframe'">
          <el-tooltip :content="$t('addDatasource')" effect="dark" placement="top">
            <i class="ksd-fs-14 el-icon-ksd-project_add"  @click="importDataSource('selectSource', currentProjectData)"></i>
          </el-tooltip>
        </div>
...
export default class DataSourceBar extends Vue {
  ...
  importDataSource (editType, project, event) {
    ...
    if (this.$store.state.capacity.maintenance_mode) {
      kapConfirm(this.$t('kylinLang.common.systemUpgradeTips'), {cancelButtonText: this.$t('kylinLang.common.continueOperate'), confirmButtonText: this.$t('kylinLang.common.tryLater'), type: 'warning', showClose: false, closeOnClickModal: false, closeOnPressEscape: false}, this.$t('kylinLang.common.tip')).then().catch(() => {
