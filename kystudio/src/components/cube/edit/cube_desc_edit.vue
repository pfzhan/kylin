<template>
<div class="cube-edit">
  <el-steps class="cube-step ksd-mt-40" :active="activeStep" space="18%" finish-status="finish" process-status="wait" align-center>
  ...
  <div class="ky-line"></div>
  <el-button class="ksd-fright ksd-mb-20 ksd-ml-10 ksd-mr-20" type="primary" v-if="activeStep !== 7" @click.native="next">{{$t('next')}}<i class="el-icon-arrow-right el-icon--right"></i></el-button>
  <el-button class="ksd-fright ksd-mb-20 ksd-ml-10 ksd-mr-20" :loading="cubeSaving" type="primary" v-if="activeStep === 7" @click.native="saveOrUpdate">{{$t('save')}}</el-button>
    <el-button class="ksd-fright ksd-mb-20 ksd-ml-10" icon="el-icon-arrow-left" v-if="activeStep !== 1" @click.native="prev">{{$t('prev')}}</el-button>
  </div>
</template>
...
import { removeNameSpace, getNameSpace, objectClone, getDiffObjInArrays, indexOfObjWithSomeKeys } from '../../../util/index'
  ...
  methods: {
    ...
    checkCubeSteps: function (index) {
      switch (index) {
        ...
        case 4:
          return this.checkRefreshSetting()
        case 5:
          return this.checkTableIndex()
        case 6:
          return this.checkConfigurationOverwrite()
        default:
          return true
      }
    },
    ...
    checkDimensions: function () {
      ...
      for (let j = 0; j < _this.cubeDetail.aggregation_groups.length; j++) {
        ...
        if (_this.cubeDetail.aggregation_groups[j] && _this.cubeDetail.aggregation_groups[j].select_rule && _this.cubeDetail.aggregation_groups[j].select_rule.hierarchy_dims && _this.cubeDetail.aggregation_groups[j].select_rule.hierarchy_dims.length > 0) {
          for (let m = _this.cubeDetail.aggregation_groups[j].select_rule.hierarchy_dims.length - 1; m >= 0; m--) {
            if (_this.cubeDetail.aggregation_groups[j].select_rule.hierarchy_dims[m].length === 0) {
              _this.cubeDetail.aggregation_groups[j].select_rule.hierarchy_dims.splice(m, 1)
            }
          }
        }
        if (_this.cubeDetail.aggregation_groups[j] && _this.cubeDetail.aggregation_groups[j].select_rule && _this.cubeDetail.aggregation_groups[j].select_rule.joint_dims && _this.cubeDetail.aggregation_groups[j].select_rule.joint_dims.length > 0) {
          for (let n = _this.cubeDetail.aggregation_groups[j].select_rule.joint_dims.length - 1; n >= 0; n--) {
            if (_this.cubeDetail.aggregation_groups[j].select_rule.joint_dims[n].length === 0) {
              _this.cubeDetail.aggregation_groups[j].select_rule.joint_dims.splice(n, 1)
            }
          }
        }
      }
      ...
      for (let i = 0; i < _this.cubeDetail.rowkey.rowkey_columns.length; i++) {
        ...
        if (_this.cubeDetail.rowkey.rowkey_columns[i].encoding.substr(0, 7) === 'integer' && (_this.cubeDetail.rowkey.rowkey_columns[i].encoding.substr(8) < 1 || _this.cubeDetail.rowkey.rowkey_columns[i].encoding.substr(8) > 8)) {
          this.$message({
            showClose: true,
            duration: 0,
            message: _this.$t('checkRowkeyInt'),
    ...
    checkMeasures: function () {
      ...
      for (let i = 0; i < this.cubeDetail.measures.length; i++) {
        if (this.cubeDetail.measures[i].function.expression === 'COUNT' && this.cubeDetail.measures[i].function.parameter.type === 'constant') {
          ...
          function loadRowTable (isDraft) {
            ...
            _this.loadRawTable({cubeName: _this.extraoption.cubeName, project: _this.selected_project}).then((res) => {
              handleSuccess(res, (data, code, status, msg) => {
                ...
                if (rawtableData) {
                  // 自动补齐新增加或者减少的列
                  if (_this.extraoption.cubeStatus === 'DISABLED' && _this.extraoption.cubeInstance.segments.length === 0) {
                    var modelAddColumn = getDiffObjInArrays(_this.$store.state.cube.cubeRawTableBaseData[_this.selected_project + _this.extraoption.modelName], rawtableData.columns, ['table', 'column'])
                    var modelRemoveColumn = getDiffObjInArrays(rawtableData.columns, _this.$store.state.cube.cubeRawTableBaseData[_this.selected_project + _this.extraoption.modelName], ['table', 'column'])
                    modelRemoveColumn.forEach((col) => {
                      var rindex = indexOfObjWithSomeKeys(rawtableData.columns, 'table', col.table, 'column', col.column)
                      rawtableData.columns.splice(rindex, 1)
                    })
                    rawtableData.columns = rawtableData.columns.concat(modelAddColumn)
                    // end
                  }
