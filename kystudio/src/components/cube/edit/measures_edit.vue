<template>
<div class="measures-edit ksd-mrl-20">
  <div class="ksd-mt-20">
    <el-button type="primary" plain size="medium" @click.native="cubeSuggestions" :disabled="isReadyCube || !getSqlResult">{{$t('sqlOutput')}}
      <common-tip   :content="$t('outputTipOne')" >
        <i class="el-icon-question ksd-fs-10"></i>
      </common-tip>
    </el-button>
    <el-button plain size="medium" icon="el-icon-setting"  @click.native="resetMeasures" :disabled="isReadyCube">{{$t('resetMeasures')}}
    </el-button>
    <common-tip :content="$t('measuresSuggestTip')" class="ksd-ml-2">
       <i class="el-icon-question ksd-fs-10"></i>
    </common-tip>
  </div>
  <el-table class="ksd-mt-16 measures-table"
    ...
    <el-table-column
      :label="$t('action')"
      width="100" v-if="!isReadyCube">
      <template slot-scope="scope">
        <common-tip :content="$t('kylinLang.common.edit')">
          <i class="el-icon-ksd-table_edit ksd-fs-16"  @click="editMeasure(scope.row)">
          </i>
        </common-tip>
        <common-tip :content="$t('kylinLang.common.delete')">
          <i class="el-icon-delete ksd-fs-16 ksd-ml-14" @click="removeMeasure(scope.row, scope.$index)">
          </i>
        </common-tip>
      </template>
    </el-table-column>
  </el-table>
   <el-button type="primary" plain size="medium" icon="el-icon-plus" :disabled="isReadyCube" @click="addMeasure" class="ksd-mb-40 ksd-mt-16">{{$t('addMeasure')}}
   </el-button>
      

  　<p class="measure-title ksd-mb-16" v-if="useColumnFamily">
      {{$t('advancedColumnFamily')}}  </p>
    <el-table 
      border v-if="useColumnFamily"
      ...
      <el-table-column
        ...
        :label="$t('action')">
          <template slot-scope="scope">
            <common-tip :content="$t('kylinLang.common.delete')">
              <i class="el-icon-delete ksd-fs-16" @click="removeColumnFamily(scope.$index)"></i>
            </common-tip>
          </template>
      </el-table-column>
    </el-table>
    <el-button v-if="useColumnFamily"  class="ksd-mt-16 ksd-mb-40" type="primary" icon="el-icon-plus"  @click="addColumnFamily" size="medium">
      {{$t('addColumnFamily')}}</el-button>


    <el-dialog width="780px" :title="$t('editMeasure')" :visible.sync="measureFormVisible" top="5%" :before-close="measureClose" :close-on-press-escape="false" :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="medium" @click="measureClose()">{{$t('cancel')}}</el-button>
        <el-button size="medium" type="primary" plain @click="checkMeasureForm" :loading="loadCheck">{{$t('kylinLang.common.submit')}}</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    measureValidSuccess: function (data) {
      ...
      if (data.measure.function.expression === 'CORR') {
        this.$set(data.measure.function.parameter, 'next_parameter', objectClone(data.nextParam))
      }
      ...
      if (this.selected_measure.function.expression === 'COUNT_DISTINCT' && this.selected_measure.function.returntype === 'bitmap' && (data.measure.function.expression !== 'COUNT_DISTINCT' || this.selected_measure.function.parameter.value !== data.measure.function.parameter.value || data.measure.function.returntype !== 'bitmap')) {
        let dictionaryIndex = -1
        let len = this.cubeDesc.dictionaries && this.cubeDesc.dictionaries.length || 0
        for (let i = 0; i < len; i++) {
