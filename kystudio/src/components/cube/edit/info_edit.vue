<template>
<div class="cube-info-edit ksd-mrl-20">
  <el-form ref="info_form" label-width="180px" :model="cubeDesc" :rules="rules">
    <h3 class="title ksd-fs-16">{{$t("basicInfo")}}</h3>
    <el-form-item :label="$t('modelName')">
      <el-input size="medium" v-model="modelDesc.name" :disabled="true"></el-input>
    </el-form-item>
    <el-form-item :label="$t('cubeName')" prop="name">
      <el-input size="medium" v-model="cubeDesc.name" :disabled="isEdit"></el-input>
    </el-form-item>
    <el-form-item :label="$t('description')">
      <el-input type="textarea" :autosize="{ minRows: 2}" v-model="cubeDesc.description"></el-input>
    </el-form-item>
    <div class="ky-line"></div>
    <h3 class="title ksd-fs-16">{{$t('noticeSetting')}}</h3>
    ...
    <el-form-item :label="$t('notificationEvents')">
      <span slot="label">{{$t('notificationEvents')}}
        <common-tip :content="$t('kylinLang.cube.noticeTip')" >
          <i class="el-icon-question ksd-fs-10"></i></common-tip>
      </span>
      <area_label size="medium" :labels="options" :placeholder="$t('kylinLang.common.pleaseSelect')" :datamap="{label: 'label', value: 'value'}" :selectedlabels="cubeDesc.status_need_notify" @refreshData="refreshNotificationEvents">
      </area_label>
    </el-form-item>
    <div class="ky-line"></div>
  </el-form>
  <h3 class="title ksd-fs-16">{{$t('optimizerInput')}}
    <common-tip :content="$t('kylinLang.cube.optimizerInputTip')" >
      <i class="el-icon-question ksd-fs-10"></i>
    </common-tip>
  </h3>
  <ul class="optimize-input">
    <li>
      <span>{{$t('modelCheck')}} </span>
      <common-tip :content="healthStatus.messages.join('<br/>')" ><i v-if="healthStatus.status!=='RUNNING' && healthStatus.status!=='ERROR'" :class="modelHealthStatus[healthStatus.status].icon" :style="{color:modelHealthStatus[healthStatus.status].color}"></i></common-tip>
      <common-tip v-if="healthStatus.status==='RUNNING'"  :content="healthStatus.messages.join('<br/>')" ><el-progress  :width="15" type="circle" :stroke-width="2" :show-text="false" :percentage="healthStatus.progress||0"></el-progress></common-tip>
      <common-tip v-if="healthStatus.status==='ERROR'" :content="healthStatus.messages.join('<br/>')" ><el-progress  :width="15" type="circle" :stroke-width="2" :show-text="false" status="exception"  :percentage="healthStatus.progress||0"></el-progress></common-tip>
    </li>
    <li class="ksd-mt-20">
      <span>2. </span>
      <span @click="collectSql" class="action_sql">{{$t('sqlPattens')}} <i class="el-icon-ksd-table_edit"></i></span>
      <span v-show="sampleSql.sqlCount>0"> ( {{sampleSql.sqlCount}} ) </span>
    </li>
  </ul>
  <el-dialog :title="$t('collectsqlPatterns')" :visible.sync="addSQLFormVisible" :before-close="sqlClose" :close-on-press-escape="false" :close-on-click-modal="false">
    <p>{{$t('kylinLang.cube.inputSqlTip1')}}</p>
    <p>{{$t('kylinLang.cube.inputSqlTip2')}}</p>
    <div :class="{hasCheck: hasCheck}">
      <kap_editor ref="sqlbox" class="ksd-mt-20" height="200" width="99%" lang="sql" theme="chrome" v-model="sampleSql.sqlString">
      </kap_editor>
    </div>
    ...
    <div class="ksd-mt-10">
      <el-button size="small" :loading="checkSqlLoadBtn" @click="validateSql" >{{$t('kylinLang.common.check')}}</el-button> 
      <el-button size="small" type="text" v-show="checkSqlLoadBtn" @click="cancelCheckSql">{{$t('kylinLang.common.cancel')}}</el-button>
    </div>
    <span slot="footer" class="dialog-footer">
      <el-button size="medium" @click="sqlClose()">{{$t('kylinLang.common.cancel')}}</el-button>
      <el-button size="medium" type="primary" plain :loading="sqlBtnLoading" @click="collectSqlToServer">{{$t('kylinLang.common.submit')}}</el-button>
    </span>
  </el-dialog>
</div>
</template>
...
export default {
  ...
  watch: {
    'cubeDesc.notify_list' () {
      this.getNotifyList = this.cubeDesc.notify_list && this.cubeDesc.notify_list.toString() || ''
    }
  },
  locales: {
