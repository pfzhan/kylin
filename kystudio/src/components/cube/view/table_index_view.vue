<template>
 <div class="ksd-mt-20 table-index-view">
  <p v-if="!cubeDesc.rawTable" class="no-table-index">{{$t('noRawTable')}}</p>
  <el-table  v-else
    :data="rawData"
    border
    class="index-table">
    <el-table-column
      ...
      <template slot-scope="scope">
        <span>{{15 * (currentPage -1) +scope.$index + 1}}</span>
      </template>
    </el-table-column>
    <el-table-column
   ...
   <pager class="ksd-fright" v-if="totalRawTable > 0" ref="pager" :perPageSize="15"  :totalSize="totalRawTable"  v-on:handleCurrentChange='currentChange' ></pager>
  </div>
</template>
<script>
...
export default {
  ...
  created () {
    if (!this.cubeDesc.rawTable) {
      ...
      this.totalRawTable = this.cubeDesc.rawTable.columns.length
