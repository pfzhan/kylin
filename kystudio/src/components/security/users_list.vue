<template>
<div class="user-list">
  <div v-if="$route.params.groupName">
    <router-link to="/security/group">
      <i class="el-icon-ksd-more_04"></i>
      {{$t('backToGroupList')}}
    </router-link>
  </div>
  <el-row class="ksd-mt-10 ksd-mb-14">
    <el-col :span="24">
      <!-- old -->
      <!-- <el-button type="primary" plain size="medium"  v-if="$store.state.system.securityProfile === 'testing' && (hasAdminProjectPermission() || isAdmin)" icon="el-icon-plus"  @click="addUser">{{$t('user')}}</el-button> -->
      <el-button type="primary" plain size="medium"  v-if="$store.state.system.securityProfile === 'testing' && (hasAdminProjectPermission() || isAdmin)" icon="el-icon-plus"  @click="showNewUser">{{$t('user')}}</el-button>
      <div style="width:200px;" class="ksd-fright" >
          <el-input size="medium" :placeholder="$t('userName')" prefix-icon="el-icon-search" v-model="serarchChar" @input="filterUserList" class="show-search-btn" >
          </el-input>
        </div>
    </el-col>
  </el-row>

  <el-alert v-if="$store.state.system.securityProfile !== 'testing'" class="ksd-mb-20"
    ...
    <el-table-column v-if="$store.state.system.securityProfile === 'testing' && (hasAdminProjectPermission() || isAdmin)"
      ...
      <template slot-scope="scope">
        ...
        <!-- <el-tooltip :content="$t('resetPassword')" effect="dark" placement="top">
          <i class="el-icon-ksd-table_reset-password ksd-mr-14" v-show="isAdmin || hasSomePermissionOfProject(selected_project)" @click="reset(scope.row)"></i>
        </el-tooltip> -->
        <el-tooltip :content="$t('resetPassword')" effect="dark" placement="top">
          <i class="el-icon-ksd-table_reset-password ksd-mr-14" v-show="isAdmin || hasSomePermissionOfProject(selected_project)" @click="changePassword(scope.row)"></i>
        </el-tooltip>
        <el-dropdown trigger="click" >
          <i class="el-icon-ksd-table_others" v-show="scope.row.username!=='ADMIN'"></i>
          <el-dropdown-menu slot="dropdown">
            <!-- old -->
            <!-- <el-dropdown-item @click.native="pushGroup(scope.row)">{{$t('groupMembership')}}</el-dropdown-item> -->
            <el-dropdown-item @click.native="assignGroup(scope.row)">{{$t('groupMembership')}}</el-dropdown-item>
            <!-- old -->
            <!-- <el-dropdown-item @click.native="edit(scope.row)">{{$t('editRole')}}</el-dropdown-item> -->
            <el-dropdown-item @click.native="editUser(scope.row)">{{$t('editRole')}}</el-dropdown-item>
<!--             <el-dropdown-item @click.native="reset(scope.row)">{{$t('resetPassword')}}</el-dropdown-item> -->
  ...
  <el-dialog @close="closeAddUser" :title="$t('addUser')" :visible.sync="addUserFormVisible" width="440px">
    <add_user :newUser="selected_user" ref="addUser" v-on:validSuccess="addUserValidSuccess"></add_user>
    <div slot="footer" class="dialog-footer">
      <el-button size="medium" @click="addUserFormVisible = false">{{$t('cancel')}}</el-button>
      <el-button size="medium" type="primary" plain @click="checkAddUserForm">{{$t('yes')}}</el-button>
    </div>
  </el-dialog>

  <el-dialog @close="closeEditRole" :title="$t('editRole')"  :visible.sync="editRoleFormVisible" width="440px">
    <edit_role :userDetail="selected_user" ref="editRole" v-on:validSuccess="editRoleValidSuccess"></edit_role>
    <div slot="footer" class="dialog-footer">
      <el-button size="medium" @click="editRoleFormVisible = false">{{$t('cancel')}}</el-button>
      <el-button size="medium" type="primary" plain @click="checkEditRoleForm">{{$t('yes')}}</el-button>
    </div>
  </el-dialog>
  ...
  <el-dialog @close="closeResetPassword" :title="$t('resetPassword')" :visible.sync="resetPasswordFormVisible" width="440px">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button size="medium" @click="resetPasswordFormVisible = false">{{$t('cancel')}}</el-button>
      <el-button size="medium" type="primary" plain @click="checkResetPasswordForm">{{$t('yes')}}</el-button>
    </div>
  </el-dialog>

  <el-dialog :title="$t('resetPassword')" :visible.sync="resetAdmin" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false" width="440px" class="reset-password">
    <el-alert class="ksd-mb-30"
  ...
  <el-dialog :title="$t('kylinLang.common.group')" :visible.sync="addGroupDialog" :close-on-press-escape="false" :close-on-click-modal="false" width="660px" class="user-group">
      <el-form>
        <el-form-item >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="medium" @click="addGroupDialog = false">{{$t('kylinLang.common.cancel')}}</el-button>
        <el-button size="medium" type="primary" plain :loading="saveBtnLoad"  :disabled="selectedGroupList.length<=0" @click="saveGroupToUser">{{$t('kylinLang.common.save')}}</el-button>
      </div>
  </el-dialog>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    // new
    async showNewUser () {
      const isSubmit = await this.callUserEditModal({ editType: 'new' })
      isSubmit && this.loadUsers()
    },
    async changePassword (userDetail) {
      const isSubmit = await this.callUserEditModal({ editType: 'password', userDetail })
      isSubmit && this.loadUsers()
    },
    async editUser (userDetail) {
      const isSubmit = await this.callUserEditModal({ editType: 'edit', userDetail })
      isSubmit && this.loadUsers()
    },
    async assignGroup (userDetail) {
      const isSubmit = await this.callUserEditModal({ editType: 'group', userDetail })
      isSubmit && this.loadUsers()
    },
    ...
    loadUsers () {
      ...
      if (this.$route.params && this.$route.params.groupName) {
        parameter.groupName = this.$route.params.groupName
    ...
    addUserValidSuccess: function (data) {
      ...
      if (this.$route.params && this.$route.params.groupName) {
        user.detail.authorities.push(this.$route.params.groupName)
      }
