<template>
    ...
    <div class="ksd_left_bar">
     ...
     <!-- reload table dialog -->
     <el-dialog :title="$t('reload')" width="660px" :visible.sync="scanRatioDialogVisible" @close="cancelReloadTable" :close-on-press-escape="false" :close-on-click-modal="false">
        <el-row :gutter="20">
      ...
      <!-- 单个采样dialog -->
      <el-dialog :title="$t('setScanRange')" :visible.sync="scanSampleRatioDialogVisible" @close="cancelLoadSample" :close-on-press-escape="false" :close-on-click-modal="false" width="660px">
        <span slot="title">{{$t('setScanRange')}} <common-tip placement="right" :content="$t('kylinLang.dataSource.collectStatice')" >
                 <i class="el-icon-question"></i></common-tip></span>
        <el-row :gutter="20">
          <el-col :span="24"><div class="grid-content bg-purple">
            <div class="tree_check_content ksd-mt-20">
              <div class="ksd-mt-20">
     ...
     <el-dialog title="Load Kafka Topic" :append-to-body="true" :visible.sync="editKafkaFormVisible" top="10%" :close-on-press-escape="false" :close-on-click-modal="false">
      ...
      <el-dialog  :title="$t('kylinLang.common.tip')" width="780px"
...
export default {
  ...
  locales: {
    ...
    'zh-cn': {'dialogHiveTreeNoData': '请点击数据源来加载源表', 'dialogHiveTreeLoading': '加载中', 'load': '加载', 'reload': '重载', 'samplingBtn': '采样', 'sampling': '表采样', 'unload': '卸载', 'loadhiveTables': '加载Hive表元数据', 'loadTables': '加载表元数据', 'setScanRange': '表采样', 'filterInputTips': '请输入hive表名进行过滤', 'loadTableJobBeginTips': '采集开始，您可以到Monitor页面查看采样进度！', 'hasCollectJob': '已有一个收集作业正在进行中，您可以去Monitor页面查看进度!', 'loadHiveTip': '<p style="font-weight: bold">加载表元数据的方式</p><p class="ksd-mt-10">选择表：可以在左侧的元数据树选择需要加载的表。每次最多可加载1000张表。</p><p>输入表：可以在右侧的加载框中直接输入‘database.table’，按回车键结束输入。输入多张表名时，请使用逗号分隔，每次最多可加载1000张表。</p>', 'loadTip': '<p style="font-weight: bold">加载表元数据的方式</p><p class="ksd-mt-10">选择表：可以在左侧的 tree选择需要加载的表。每次最多可加载1000张表。</p><p>输入表：可以在右侧的加载框中直接输入‘database.table’，按回车键结束输入。输入多张表名时，请使用逗号分隔，每次最多可加载1000张表。</p>', 'access': '权限', 'selectedHiveValidateFailText': '请输入完整表名\'database.table\'。', newDataSource: '新数据源', 'loadKafkaTopic': '设置Kafka主题', dataSourceTypeCheckTip: '请选择数据源类型', singleSourceTip: '每个项目支持一种数据源类型，选定后将无法修改。'}
  }
}
</script>
<style lang="less" >
