...
export const render = {
  ...
  table: {
    render (h, { node, data, store }) {
      const { label, tags, dateRange, isTopSet, isHideTopSet, isShowCancelSelected, isSelected } = data
      const nodeClass = {
        class: [
          'frontground',
          'table',
      ...
      return (
        <div>
          <div {...nodeClass}>
            ...
            <div class="right">
              { isHideTopSet ? null : (<span class="tree-icon" slot="reference">
...
export function getDatasourceObj (that, sourceType) {
  const { currentSelectedProject, customTreeTitle } = that
  ...
  return {
    id: sourceType,
    label: customTreeTitle !== '' ? `${that.$t(customTreeTitle)}` : `${sourceNameStr} ${that.$t('source')}`,
...
export function getTableObj (that, database, table, ignoreColumn, isHideTopSet) {
  ...
  const tableObj = {
    ...
    isTopSet: table.top,
    isHideTopSet: isHideTopSet,
    isHideFactIcon: that.hideFactIcon,
    isShowCancelSelected: that.isShowCancelSelected,
