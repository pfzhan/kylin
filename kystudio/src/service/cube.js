...
export default {
  ...
  rebuildCube: (cube) => {
    return Vue.resource(apiUrl + 'cubes/' + cube.cubeName + '/segments/build').update(cube.timeZone)
  },
  rebuildStreamingCube: (cubeName) => {
    return Vue.resource(apiUrl + 'cubes/' + cubeName + '/segments/build_streaming').update({
      sourceOffsetStart: 0,
  ...
  checkCubeNameAvailability: (para) => {
    return Vue.resource(apiUrl + 'cubes/validate/' + para.cubeName).get()
  },
  calCuboid: (cubeDesc) => {
