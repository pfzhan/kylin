...
exports.default = {
  common: {
    ...
    enterAlias: '输入别名',
    pleaseSelect: '请选择',
    pleaseSelectOrSearch: '请搜索或者选择',
    pleaseSelectUserName: '请选择用户',
    pleaseSelectUserGroup: '请选择组',
    pleaseSelectUserOrGroupName: '请选择用户名或组名',
    pleaseInputUserName: '请搜索或者选择用户名',
    pleaseInputUserGroup: '请搜索或者选择用户组名',
    ...
    dot: '。',
    colon: '：',
    sqlLimitTip: '当前默认仅展示前 100 行 SQL 语句，请点击复制按钮获取完整的查询语句',
  ...
  model: {
    ...
    checkOption: '检测选项：',
    checkModel: '模型检测',
    buildIndexSuccess: '构建索引任务提交成功，可以在任务监控页面查看。',
    buildIndexSuccess1: '新的{indexType}构建完成后将服务于查询。',
    saveIndexSuccess: '您已成功保存{indexType}。',
    buildIndexFail1: '构建索引任务提交失败，尚未设置模型 {modelName} 的数据加载范围。',
    buildIndexFail2: '{indexType}无变化，无需构建索引。',
    scanRangeSetting: '时间范围设置',
    sameModelName: '已存在同名的模型。',
    modelNameTips: '1. 在一个项目中，模型名称需要是唯一的。<br/> 2. 模型创建后，支持重命名。',
    partitionDateTip: '1. 时间分区列是可选的；如果需要经常全量构建, 请将分区时间列留空。<br/> 2. 时间分区列中应该为时间值。（类型可以是 date，timestamp，string，varchar，int，integer，bigint）',
    partitionSplitTip: '增加多级分区列。',
    secondaryPartitionWarring: '设置一级分区列之前，请先设置时间分区列。',
    partitionDateTipNeedTip: '流式数据模型（Kafka数据源）必须设置时间分区列。',
    ...
    modelCheckOptionsTips: '完整的模型检测包括：模型状态检测、事实表采样、维度表采样。<br/>模型状态检测主要检查数据的分布不均等问题，表采样更多服务于cube的维度优化等。',
    ...
    modelCheck: '模型检测将会检查数据中可能的问题，比如数据分布不均匀；并服务于后续的 cube 设计优化功能。<br/>完整的模型检测包括：模型状态检测、事实表采样、维度表采样。<br/>1. 模型状态检测主要检查数据的分布不均等问题。<br/>2. 表采样更多服务于cube的维度优化等。',
    ...
    reuseCC2: '"里存在，',
    reuseClick: '点此复用。',
    sameCCName1: '当前模型下，可计算列名"',
    sameCCName2: '"已被使用，请重新命名您想创建的可计算列。',
  ...
  cube: {
    ...
    noDims: '暂未添加维度',
    inputSqlTip1: '1. 下面展示了输入的所有 SQL 语句，每句以“;”作为分割。',
    inputSqlTip2: '2. 输入的不当 SQL 语句将被忽视，不会对后续步骤产生作用。',
    readyCubeTip: '已包含 segment 的 cube 不支持编辑，避免cube元数据不一致。',
    disableCubeEditTip: 'Cube {cubename} 包含数据。为了保证数据定义一致， 修改 cube 定义前，请先清理 cube 数据。',
    // disableCubeSaveEditTip: '原Cube 定义 {cubename} 与最新编辑不一致， 请先清理 cube 数据再修改cube定义，或者避免修改cube中包含的列。',
    ...
    orderedbytesTip: '适用于所有类型。它会在编码时保持数据的顺序，也是大多数情况下的默认的编码。',
    sameCubeName: '已经存在同名的Cube',
    inputCubeName: '请输入Cube名称',
    addCube: '添加Cube',
    cubeHasJob: '此cube已有在执行的 cube 构建任务，因此当前操作不支持。',
    selectModelName: '请选择一个模型',
    optimizerInputTip: '什么是优化器？优化器可以根据输入内容，自动推荐维度、度量和维度优化的方式。',
    rowkeyTip: '<h4>什么是 Rowkeys?</h4><p>Rowkeys 定义了维度列之间如何组织在一起</p><h4>是否按该列分散存储? </h4><p>若设为"true", Cube 数据将按该列值分散存储</p><h4>Rowkey 编码</h4><ol><li>"dict" 适用于大部分字段，默认推荐使用。但在超高基情况下，可能引起内存不足的问题。</li><li>"boolean" 适用于内容为：true, false； TRUE, FALSE； True, False； t, f； T, F； yes, no； YES, NO； Yes, No； y, n； Y, N； 1, 0的字段。</li><li>"integer" 适用于字段值为整数字符，支持的整数区间为[ -2^(8*N-1), 2^(8*N-1)]。 </li><li>"int" 已弃用, 请使用最新的integer编码。</li><li>"date" 适用于字段值为日期字符, 支持的格式包括yyyyMMdd、yyyy-MM-dd、yyyy-MM-dd HH:mm:ss、yyyy-MM-dd HH:mm:ss.SSS, 其中如果包含时间戳部分会被截断。</li><li>"time" 适用于字段值为时间戳字符, 支持范围为[ 1970-01-01 00:00:00, 2038/01/19 03:14:07], 毫秒部分会被忽略。 time 编码适用于 time, datetime, timestamp 等类型。</li><li>"fix_length" 适用于超高基场景, 将选取字段的前N个字节作为编码值, 当N小于字段长度, 会造成字段截断, 当N较大时, 造成RowKey过长, 查询性能下降。只适用于 varchar 或 nvarchar 类型。</li><li>"fixed_length_hex" 适用于内容为十六进制字符的字段，比如 1A2BFF 或者 FF00FF（每两个字符需要一个字节）。</li></ol>',
    dataOriented: '模型优先',
    businessOriented: '业务优先',
    defaultOriented: '默认',
    optimizeStrategy: '优化策略',
    businessOrientedTip: '“业务优先”策略下，输入 SQL 尚未完成，维度优化无法完成。请选择其他策略或输入您的常用 SQL。',
  ...
  system: {
    ...
    statement: '您已经购买 Kyligence Enterprise 企业版产品及服务。如果您在使用过程中遇到任何问题，欢迎随时与<a href="mailto:g-ent-lic@kyligence.io">我们沟通</a>，我们将持续为您提供优质的产品及服务！',
    hostName: '主机名',
    type: '类型',
    status: '状态'
  },
  guide: {
    addProjectTip: '添加项目时，请注意选择项目类型。如果您希望作为多维建模专家进行分析，请选择 AI 增强模式。智能模式更适用于直接对接 BI，系统将分批加速 BI 发出的 SQL 语句。',
    addProjectTipAuto: '添加项目时，请注意选择项目类型。智能模式更适用于直接通过BI探索数据，系统将透明地加速 BI 发出的 SQL 语句。如果您希望作为多维建模专家进行分析，则选择 AI 增强模式。',
    ...
    speedTipAuto1: '默认所有由系统管理员发送的 SQL 语句都会被加速。',
    speedTipAuto2: '点击 立即加速 按钮，系统会立即触发任务去加速所有待加速的SQL。',
    monitorTip: '触发数据加载任务。之后你可以在监控页面查看相关任务。本任务执行一般需要 5 分钟，您可以退出新手指导稍后来查看任务结果。',
  ...
  capacity: {
    ...
    capacityFailTip: '已使用数据量无法获取，请尝试在顶栏刷新。若刷新后仍未恢复正常，请尽快联系 Kyligence 销售人员。',
    nodesFailTip: '已使用节点无法获取，请尝试在顶栏刷新。若刷新后仍未恢复正常，请尽快联系 Kyligence 销售人员。',
    bothCapacityAndNodesFail: '已使用数据量和节点无法获取，请尝试在顶栏刷新。若刷新后仍未恢复正常，请尽快联系 Kyligence 销售人员获取帮助。'
  },
  kybotXY: {
