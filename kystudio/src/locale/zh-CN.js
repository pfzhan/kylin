...
exports.default = {
  common: {
    ...
    pleaseFilterByCubeName: '输入cube名称筛选',
    enterAlias: '输入别名',
    pleaseSelect: '请选择',
  ...
  model: {
    ...
    modelCheck: '模型检测将会检查数据中可能的问题，比如数据分布不均匀；并服务于后续的cube设计优化功能。<br/>完整的模型检测包括：模型状态检测、事实表采样、维度表采样。<br/>1. 模型状态检测主要检查数据的分布不均等问题。<br/>2. 表采样更多服务于cube的维度优化等。',
  ...
  cube: {
    ...
    disableCubeEditTip: 'Cube {cubename} 包含数据。为了保证数据定义一致， 修改cube定义前，请先清理 cube 数据。',
  ...
  guide: {
    addProjectTip: '添加项目时，请注意选择项目类型。如果您希望做为多维建模专家进行分析，请选择专家模式。智能模式更适用于直接对接BI，系统将分批加速BI发出的SQL语句。',
    addProjectTipAuto: '添加项目时，请注意选择项目类型。智能模式更适用于直接通过BI探索数据，系统将透明地加速BI发出的SQL语句。如果您希望做为多维建模专家进行分析，则选择专家模式。',
    ...
    speedTipAuto: '系统将筛选过的SQL语句放在待加速列表，您可以通过调整筛选规则控制加速的查询数量。',
    speedTipAuto1: '默认所有由系统管理员发送的SQL语句都会被加速。',
    ...
    loadTableTip: 'Kyligence 企业数据分析平台需要连接您的源数据系统。选择你所需的数据集并且从中同步表的元数据。这里我们选用SSB数据集为后续建模做准备。'
  },
  kybotXY: {
