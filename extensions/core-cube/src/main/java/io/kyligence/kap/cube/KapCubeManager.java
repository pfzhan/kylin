...
public class KapCubeManager implements IRealizationProvider {

    private static String ALPHA_NUM = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";

    private static int HBASE_TABLE_LENGTH = 10;
    public static final Serializer<KapCubeInstance> CUBE_SERIALIZER = new JsonSerializer<KapCubeInstance>(KapCubeInstance.class);

    private static final Logger logger = LoggerFactory.getLogger(KapCubeManager.class);

    // static cached instances
    private static final ConcurrentHashMap<KylinConfig, KapCubeManager> CACHE = new ConcurrentHashMap<KylinConfig, KapCubeManager>();

    public static KapCubeManager getInstance(KylinConfig config) {
        KapCubeManager r = CACHE.get(config);
        if (r != null) {
            return r;
        }

        synchronized (org.apache.kylin.cube.CubeManager.class) {
            r = CACHE.get(config);
            if (r != null) {
                return r;
            }
            try {
                r = new KapCubeManager(config);
                CACHE.put(config, r);
                if (CACHE.size() > 1) {
    ...
    private KapCubeManager(KylinConfig config) throws IOException {
    ...
    public CubeInstance dropCube(String cubeName, boolean deleteDesc) throws IOException {
        ...
        if (deleteDesc && cube.getDescriptor() != null) {
            KapCubeDescManager.getInstance(config).removeCubeDesc(cube.getDescriptor());
        }
    ...
    private CubeInstance updateCubeWithRetry(CubeUpdate update, int retry) throws IOException {
        ...
        try {
            getStore().putResource(cube.getResourcePath(), (KapCubeInstance) cube, CUBE_SERIALIZER);
    ...
    private synchronized CubeInstance reloadCubeLocalAt(String path) {
        ...
        KapCubeInstance cubeInstance;
        try {
            cubeInstance = store.getResource(path, KapCubeInstance.class, CUBE_SERIALIZER);

            CubeDesc cubeDesc = KapCubeDescManager.getInstance(config).getCubeDesc(cubeInstance.getDescName());
            if (cubeDesc == null)
    ...
    private org.apache.kylin.cube.CubeManager.CubeChangeListener listener;

    public void setCubeChangeListener(org.apache.kylin.cube.CubeManager.CubeChangeListener listener) {
