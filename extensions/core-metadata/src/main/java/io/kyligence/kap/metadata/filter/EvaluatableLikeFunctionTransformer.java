...
public class EvaluatableLikeFunctionTransformer {
    ...
    public static TupleFilter transform(TupleFilter tupleFilter, Set<TblColRef> likeColumnsCollector) {
        ...
        if (tupleFilter instanceof CompareTupleFilter) {
                ...
                for (TupleFilter child : builtInFunctionTupleFilter.getChildren()) {
                    ...
                    if (child instanceof ColumnTupleFilter) {
                        ...
                        if (pattern.length() < KapConfig.getInstanceFromEnv().getParquetFuzzyIndexLength() + 2) {
                            throw new IllegalArgumentException("The like pattern: " + pattern + " is too short, minimal length: " + KapConfig.getInstanceFromEnv().getParquetFuzzyIndexLength());
                        }
                    } else {
            ...
            while (childIterator.hasNext()) {
                TupleFilter transformed = transform(childIterator.next(), likeColumnsCollector);
                if (transformed != null) {
