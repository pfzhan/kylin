...
KAP_SPARK_DRIVER_DEFAULT_PORT=7071
KAP_SPARK_UI_DEFAULT_PORT=4041
...
then
    ...
    new_spark_ui_port=`expr ${KAP_SPARK_UI_DEFAULT_PORT} + ${OFFSET}`
    add_or_reset "kap.storage.columnar.spark-driver-port" ${new_spark_port} ${KYLIN_CONFIG_FILE}
    add_or_reset "kap.storage.columnar.spark-conf.spark.ui.port" ${new_spark_ui_port} ${KYLIN_CONFIG_FILE}

    echo "New kylin port is : ${new_kylin_port}"
    echo "New Spark Driver port is : ${new_spark_port}"
    echo "New Spark Driver UI port is : ${new_spark_ui_port}"
