...
public class DDLTest {
    ...
    public void testCreateTable() {
        final CreateTable<?> create = CreateTable.create("pufa", "xx").columns(new ColumnWithType("a", "int"));
        assertEquals("CREATE TABLE if not exists `pufa`.`xx`(a int)", create.toSql());

        // test create table with nullable
        final CreateTable<?> create2 = CreateTable.create("pufa", "xx").columns(new ColumnWithType("a", "int", true));
    ...
    public void testInsertIntoValue() throws SQLException {
        final InsertInto insertInto = InsertInto.insertInto("pufa", "xx").set("column1", 42).set("column2", "xyz");
        assertEquals("INSERT INTO `pufa`.`xx` (column1, column2) VALUES (42, 'xyz')", insertInto.toSql());
    }
