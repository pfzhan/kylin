...
public class ClickHouseTableStorageMetric {
    ...
    public void collect(boolean skipDownNode) {
        if (load)
            return;
        partitionSize = nodes.parallelStream().collect(Collectors.toMap(node -> node, node -> {
            ...
            try (ClickHouse clickHouse = new ClickHouse(SecondStorageNodeHelper.resolve(node))) {
                return clickHouse.query(ClickHouseSystemQuery.queryTableStorageSize(),
                        ClickHouseSystemQuery.TABLE_STORAGE_MAPPER);
    ...
    public Map<String, Long> getByPartitions(String database, String table, SegmentRange segmentRange,
            String dateFormat) {
        ...
        if (segmentRange.isInfinite()) {
            ...
            SimpleDateFormat partitionFormat = new SimpleDateFormat(dateFormat,
    ...
    public Map<String, Long> getByPartitions(String database, String table, List<String> partitions) {
        ...
        return nodes.stream()
                .collect(Collectors.toMap(node -> node, node -> sumNodeTableSize(node, database, table, partitionSet)));
    }

    private long sumNodeTableSize(String node, String database, String table, Set<String> partitionSet) {
        return partitionSet.stream().flatMap(partition -> partitionSize.get(node).stream()
                .filter(size -> Objects.equals(database, size.getDatabase()) && Objects.equals(table, size.getTable())
                        && Objects.equals(partition, size.getPartition()))
                .map(ClickHouseSystemQuery.PartitionSize::getBytes)).reduce(Long::sum).orElse(0L);
    }
}
