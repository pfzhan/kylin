...
import org.apache.spark.sql.execution.datasources.jdbc.ShardJDBCRelation
...
case class ShardJDBCScan(
    relation: ShardJDBCRelation,
    prunedSchema: StructType,
    pushedFilters: Array[Filter],
    pushedStatement: SingleSQLStatement) extends V1Scan {

  lazy val requiredColumns: Array[String] = prunedSchema.map(_.name).toArray
  ...
  override def toV1TableScan[T <: BaseRelation with TableScan](context: SQLContext): T = {
    new BaseRelation with TableScan {
      ...
      override def buildScan(): RDD[Row] = {
        relation.buildScan(requiredColumns, pushedFilters, defaultStatement.asInstanceOf[SingleSQLStatement])
      }
    }.asInstanceOf[T]
  }
  ...
  lazy val defaultStatement: SQLStatement = {
    if (pushedStatement != null) {
      pushedStatement
    } else {
      SingleSQLStatement(requiredColumns, pushedFilters, relation.jdbcOptions)
    }
  }
}
