...
object ShardJDBCRDD extends Logging {
   ...
   */
  private def resolveTable(options: JDBCOptions): StructType = {
    val url = options.url
  ...
  /**
   * Returns a factory for creating connections to the given JDBC URL of each shard.
   *
   * @param options - JDBC options that contains url, table and other information.
   */
  ...
  def scanTable(
    ...
    new ShardJDBCRDD(
      sc,
      createConnectionFactoryByShard(options),
